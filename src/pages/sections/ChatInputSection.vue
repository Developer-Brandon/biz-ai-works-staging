    ...
    <div class="chat-input-field">
      ...
      <div class="input-bottom-section no-drag">
        ...
        <div class="ai-model-info">
          ...
          <template v-if="dataStore.selectedModel !== '모델선택'">
              ...
              >
                ...
                <div v-if="dataStore.modelLoadError" class="dropdown-error">
                  <div class="error-content">
                    <span>⚠️ {{ dataStore.modelLoadError }}</span>
...
/**
 * 🏪 상태 관리 저장소 초기화
 * - dataStore: UI 상태 (드롭다운, 모델 선택 등)
 * - dataStore: 데이터 (메시지, AI Agent, 모델 목록)
...
// ==================== Computed Properties ====================
/**
 * 🔀 계산된 속성: AI Agent 버튼 disabled 여부
 *
 * Vue 2 vs Vue 3:
 * - Vue 2: computed() { return ... }
 * - Vue 3: computed(() => ...) ← 화살표 함수로 더 간결함
 *
 * 로직:
 * - currentRoomType === "model" → disabled (일반 대화방에서는 Agent 변경 불가)
 * - 그 외 → enabled
 */
...
/**
 * 🔀 계산된 속성: AI Model 드롭다운 disabled 여부
 *
 * 로직:
...
const toggleAiAgentDropdown = () => {
  ...
  //   console.log("⚠️ AI Agent 버튼이 disabled 상태입니다");
  //   return;
  // }
  console.log("🔄 에이전트 드롭다운 토글");
...
const selectAiAgent = async (aiAgent, aiAgentData) => {
  ...
  try {
    ...
    dataStore.setError(error.message || "Agent 선택에 실패했습니다");
  } finally {
 ...
 * - Vue 3: const toggleModelDropdown = async () => { ... } ← 더 직관적
 *
 * 역할:
 * 1. disabled 상태 확인
 * 2. 드롭다운 토글
...
/**
 * ⏱️ calculateTypingDuration - 타이핑 애니메이션 시간 계산
 *
 * 매개변수:
...
const handleSendMessage = async () => {
  ...
  try {
    ...
    if (dataStore.isAiAgentSelected) {
      ...
      console.log(
        "✅ 에이전트 dataStore.currentRoomId:",
        dataStore.currentRoomId,
      );

      if (dataStore.currentRoomId) {
        agentData.roomId = dataStore.currentRoomId;
        dataStore.setCreatingNewRoom(false); // ← 여기서 플래그 ON
      } else {
        agentData.roomId = "";
        console.log("🆕 새 채팅 생성됨 - 플래그 ON");
        dataStore.setCreatingNewRoom(true); // ← 여기서 플래그 ON
      }
      ...
      if (dataStore.selectedModel === "모델선택") {
      ...
      if (!dataStore.currentRoomId) {
        ...
        // ============================================================
        console.log("📌 기존 채팅방 유지:", dataStore.currentRoomId);
        ...
        messageData = {
          ...
          currentProvider: modelData.provider,
          executionMode: "chat",
          roomId: dataStore.currentRoomId, // ✅ 저장된 roomId 사용
          files: dataStore.attachedFiles,
        };
    ...
    // ============================================================
    // 7️⃣ 메세지 응답받고 마지막에 메세지 초기화 및 파일첨부 초기화
    // ============================================================
    inputMessage.value = "";
    ...
    dataStore.setError(error.message || "메시지 전송에 실패했습니다.");
...
const handleStreamingEvent = (eventType, eventData) => {
  switch (eventType) {
    ...
    case "error":
      ...
      dataStore.setError(eventData.message || "오류가 발생했습니다.");
      break;
  }
};
 ...
 * - Vue 3: watch(() => ..., (newVal) => { ... }) ← 더 명확하고 유연함
 *
 * 언제 호출되나?
 * 1️⃣ selectAiAgent()에서 aiAgentTagList 저장할 때
 * 2️⃣ 채팅방 전환 시 aiAgentTagList 변경될 때
 * 3️⃣ MainPage.vue에서 Agent 태그 갱신할 때
 *
 * 역할:
 * - aiAgentTagList[0].id 감지
 ...
 *
 * 흐름:
 * 1️⃣ 사용자 입력 또는 inputMessage 변경
 * 2️⃣ watch 감지
 * 3️⃣ emit('update:modelValue', newVal) 발생
 * 4️⃣ MainPage: chatMessage.value 업데이트 ✨
 *
 * ⚠️ 주의:
...
onMounted(async () => {
  ...
  try {
    ...
    // 1️⃣ 드롭다운 닫기 이벤트 리스너 등록
    // ============================================================
    console.log("1️⃣ 드롭다운 이벤트 리스너 등록");
